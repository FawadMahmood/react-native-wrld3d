...
const MapComponent = (props, forwardedRef) => {
  ...
  const getBuildingInformation = async _ref3 => {
    ...
    const message = await _reactNative.NativeModules['MapViewModule'].getBuildingInformation(viewId.current, location.longitude, location.latitude, animateToBuilding, duration, zoomLevel, highlight);
    return message;
  };
  ...
  const setBuildingHighlight = async _ref4 => {
    ...
    const message = await _reactNative.NativeModules['MapViewModule'].addBuildingHighlight(viewId.current, location.longitude, location.latitude);
    return message;
  }; //mapevents
